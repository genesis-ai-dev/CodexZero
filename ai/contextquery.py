...
class DatabaseContextQuery:
    ...
    def __init__(self, source_verses: List[Tuple[int, str]], target_verses: List[Tuple[int, str]], coverage_weight: float = 0.5):
        """
        Initialize with sparse verse data.
        
        Args:
            source_verses: List of (verse_index, verse_text) tuples
            target_verses: List of (verse_index, verse_text) tuples
            coverage_weight: Weight for coverage vs BM25 scoring
        """
        self.coverage_weight = coverage_weight
        
        # Create dictionaries for efficient lookup
        self.source_dict = {idx: text for idx, text in source_verses}
        self.target_dict = {idx: text for idx, text in target_verses}
        
        # Find valid verse pairs (verses that exist in both source and target)
        self.valid_indices = sorted(list(
            set(self.source_dict.keys()) & set(self.target_dict.keys())
        ))
        
        print(f"DatabaseContextQuery: {len(source_verses)} source verses, {len(target_verses)} target verses")
        print(f"Found {len(self.valid_indices)} valid verse pairs")
    ...
    def _preprocess_bm25(self):
        ...
        for idx in self.valid_indices:
            doc = self._normalize_text(self.source_dict[idx])
            words = doc.split()
            self.doc_lengths[idx] = len(words)
            self.term_freqs[idx] = Counter(words)
    ...
    def search_by_text(self, query_text: str, top_k: int = 5, min_examples: int = 3, coverage_threshold: float = 0.9, exclude_idx: Optional[int] = None) -> List[Tuple[int, str, str, float]]:
        ...
        while len(results) < top_k and restart_count < 3:
            ...
            source_text = self.source_dict[best_idx]
            target_text = self.target_dict[best_idx]
            coverage = self._compute_coverage(query_text, source_text)
            
            # Return with 1-based index for compatibility
            results.append((best_idx + 1, source_text, target_text, coverage))
            used_indices.add(best_idx)
            ...
            if covered_substring:
                ...
                print(f"Result {len(results)}: Selected verse {best_idx + 1}, coverage: {coverage:.2f}, cumulative: {cumulative_coverage:.2f}")
            
            # Check if we should stop early
            if len(results) >= min_examples and cumulative_coverage >= coverage_threshold:
                print(f"Stopping early: {len(results)} examples with {cumulative_coverage:.2f} coverage")
                break
